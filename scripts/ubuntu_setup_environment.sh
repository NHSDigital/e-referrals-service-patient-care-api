...
#Installing python 3.11.9 with pyenv
echo "Installing python 3.11.9 with pyenv ..."
if pyenv versions | grep -Fq "3.11.9" ; then
    echo "Pyenv has already got Python 3.11.9 installed."
else
    if pyenv install 3.11.9 ; then
        echo "Pyenv installed Python 3.11.9 successfully."
    else
        echo "Pyenv did NOT install Python 3.11.9 successfully."
        exit 1
    fi
fi


#Creating Apigee environment with Python 3.11.9
echo "Creating APIM-PC environment with Python 3.11.9 ..."
if pyenv versions | grep -q ".*apim-pc" ; then
    echo "A Python virtualenv named 'apim-pc' already exists."
else
    if pyenv virtualenv 3.11.9 apim-pc ; then
        echo "A Python 3.11.9 virtualenv named 'apim-pc' was created."
    else
        echo "A Python 3.11.9 virtualenv named 'apim-pc' was NOT created."
        exit 1
    fi
fi
...
if pyenv local apim-pc ; then
    echo "APIM-PC enviroment activated."
else
    echo "APIM-PC enviroment NOT activated."
    exit 1
fi
...
if poetry -V 2> /dev/null ; then
    echo "Poetry is already installed on APIM-PC environment."
else
    if pip install poetry ; then
        echo "Poetry was installed on APIM-PC environment."
    else
        echo "Poetry was NOT installed on APIM-PC environment."
        exit 1
    fi
fi
...
printf "\n\nSetup successfull! \n\
The APIM-PC environment is now set with the .python-version file,\n\
\033[1mso there's no need to run $ pyenv activate APIM-PC\033[0m,\n\
cd'ing to the repo should suffice.\n\
Please \033[1mopen another terminal\033[0m to update your \$PATH and run:\n\
\033[1m$ pyenv version\033[0m, which should output 'APIM-PC'\n\
